...
public class NeuralNetwork : IComparable<NeuralNetwork>
{
    ...
    public NeuralNetwork CrossOver(NeuralNetwork secondParent)
    {
        ...
        for (int i = 0; i < weights.Length; i++)
        {
            for (int j = 0; j < weights[i].Length; j++)
            {
                for (int k = 0; k < weights[i][j].Length; k++)
                {
                    ...
                    {
                        child.weights[i][j][k] = secondParent.weights[i][j][k];
                    }
                }
            }
        }
            ...
            for (int j = 0; j < weights[i].Length; j++)
            {
                for (int k = 0; k < weights[i][j].Length; k++)
                {
                    ...
                    if (mutationRate < rnd)
                    {
                        ...
                        float randomNumber = UnityEngine.Random.Range(0f, 8);
    ...
    //comparing two networks by fitness
    public int CompareTo(NeuralNetwork other)
    {
        Debug.Log("Porównuję "+ fitness +" z "+other.fitness);
    ...
    public void SetMutationRate(float mutationRatio)
    {
        mutationRate = mutationRatio;
    }
}
